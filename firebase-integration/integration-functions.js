...
function firebaseFunctions({ dist, packageJson } = {}) {
  let _config;
  return {
    name: 'astro-firebase',
    hooks: {
      ...
      'astro:build:done': async ({ routes, dir }) => {      
        ...
        try {
          ...
          fs.writeFileSync(firebaseJsonURL, JSON.stringify({hosting: { rewrites }}, null, 2));
        }
        ...
        fs.writeFileSync(new URL('./package.json', dir), JSON.stringify({
          type: "module", 
          engines: {
            node: '16'
          },
